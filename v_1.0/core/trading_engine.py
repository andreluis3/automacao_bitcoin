"""
trading_engine.py — Engine de trading com logs profissionais integrados.

Novidades vs versão anterior:
- Integra BotLogger (logs em pastas separadas)
- Expõe set_aggressive_mode() e set_lateral_scalping() para o BotController
- Logs de decisão, ordem, resultado, estado e confluência em cada operação
"""
...
class BinanceExecutionAdapter(BaseExecutionAdapter):
    ...
    def buy_quote(self, quote_brl: float, price_brl: float) -> tuple[float, float]:
        ...
        order = self.client.create_order(
            symbol="BTCBRL", side="BUY", type="MARKET",
            quoteOrderQty=round(amount, 2),
        )
        qty = sum(float(f.get("qty", 0.0)) for f in order.get("fills", []))
        if qty <= 0:
    ...
    def sell_quantity(self, quantity_btc: float, price_brl: float) -> tuple[float, float]:
        ...
        if qty <= 0 or price_brl <= 0:
            return 0.0, 0.0
        order = self.client.create_order(
            symbol="BTCBRL", side="SELL", type="MARKET",
...
class TradingEngine:
    def __init__(
        ...
        # Para calcular duração dos trades no log
    ...
    def set_aggressive_mode(self, enabled: bool) -> None:
        """Liga modo agressivo: entra se EMA9 > EMA21 sem exigir crossover."""
        self.strategy.set_aggressive_mode(bool(enabled))

    def set_lateral_scalping(self, enabled: bool) -> None:
        """Liga scalping por Bollinger Bands em mercado lateral."""
    ...
    def set_regime_threshold(self, threshold: float) -> None:
        ...
        0.000002 = muito sensível | 0.000005 = padrão | 0.00001 = conservador
        """
    ...
    def set_risk_profile(self, profile_name: str) -> None:
        ...
        if key == "conservador":
            self.config.risk = RiskConfig(
                risk_per_trade_pct_base=2.0, risk_per_trade_pct_aggressive=3.0,
                risk_per_trade_pct_defensive=1.0, max_exposure_pct=30.0, min_order_value_brl=35.0,
            )
            ...
            self.config.risk = RiskConfig(
                risk_per_trade_pct_base=2.5, risk_per_trade_pct_aggressive=4.0,
                risk_per_trade_pct_defensive=1.25, max_exposure_pct=40.0, min_order_value_brl=35.0,
            )
    ...
    def _equity_for_risk(self, price_brl: float) -> float:
        ...
        if self.accumulation_mode:
            return max(0.0, eq - self.safe_reserve_brl)
        return max(0.0, eq)
    ...
    def _update_risk_regime(self, signal_ctx: Optional[dict] = None) -> None:
        ...
        if self.consecutive_losses >= 3:
            ...
            if pf > 1.3 and str((signal_ctx or {}).get("active_mode") or "") == "tendencia":
    ...
    def on_price(self, price_brl: float, now: Optional[datetime] = None,
        ...
        # Log de mercado (a cada tick, mas arquivo — não terminal a cada vez)
        if self.bot_log:
            try:
                self.bot_log.market(
                    price=price, volume=volume,
                    ema9=e9, ema21=e21,
                    atr=signal_ctx.get("atr"),
                    regime=self.active_strategy_mode,
                    timestamp=now,
                )
            except Exception:
                pass
        ...
        if self.position:
            ...
            if self._can_scale_position(signal_ctx):
                ...
                if add_pct > 0:
                    ...
                    scaled = self._scale_in_position(price, capital_total * add_pct, now, signal_ctx)
                    if scaled.get("trade"):
        ...
        if signal_ctx.get("signal") != "buy":
            ...
            # Log de decision (a cada N ticks para não poluir)
            if self.bot_log and self.cross_filtrados % 20 == 0:
                try:
                    self.bot_log.decision(
                        ...
                        regime=self.active_strategy_mode,
                        signal="none", reason=str(signal_ctx.get("reason") or ""),
                    )
                except Exception:
                    pass
        ...
        if self.bot_log:
            try:
                self.bot_log.decision(
                    ...
                    volume=volume, avg_volume=float(signal_ctx.get("avg_volume") or 0.0),
                    regime=self.active_strategy_mode,
                    signal="buy", reason=str(signal_ctx.get("reason") or ""),
                )
            except Exception:
                pass
    ...
    def _open_position(self, plan: PositionPlan, now: datetime,
        ...
        if self.bot_log:
            try:
                self.bot_log.order(
                    side="BUY", price=plan.entry_price, qty_btc=btc,
        ...
        self.last_trade = {
            "side": "BUY", "price": plan.entry_price, "btc": btc,
            "fee_brl": fee_brl, "timestamp": now.isoformat(timespec="seconds"),
            "reason": motivo_entrada, "risk_per_trade_pct": plan.risk_per_trade_pct,
            "active_mode": str((strategy_context or {}).get("active_mode") or self.active_strategy_mode),
        }
        return {
            "trade": True, "side": "BUY", "btc": btc,
            "risk_pct": plan.risk_pct, "entry": plan.entry_price,
            "risk_per_trade_pct": plan.risk_per_trade_pct, "reason": motivo_entrada,
        }
    ...
    def _close_position(self, price_brl: float, motivo_saida: str, now: datetime,
        ...
        if self.trade_manager:
            try:
                self.trade_manager.fechar_trade(
                    modo=self.mode, preco_saida=price_brl, saldo_depois=saldo_depois,
                    quantidade=pos.quantity, lucro_brl=pnl_brl, lucro_percentual=pnl_pct,
        ...
        if self.bot_log:
            try:
                self.bot_log.order(
                    side="SELL", price=price_brl, qty_btc=pos.quantity,
                    value_brl=amount_brl, fee_brl=fee_brl,
                    reason=motivo_saida, status="EXECUTADA",
                )
                self.bot_log.result(
                    entry_price=pos.entry_price, exit_price=price_brl,
        ...
        return {
            "trade": True, "side": "SELL", "amount_brl": amount_brl,
    ...
    def _can_scale_position(self, signal_ctx: dict) -> bool:
        ...
        if self.last_trade_at:
            if (datetime.utcnow() - self.last_trade_at) < timedelta(
    ...
    def _signal_strength(self, signal_ctx: dict) -> float:
        ...
        return max(0.0, min(1.0,
            (dist * 20.0) + (slope / max(price, 1.0) * 200.0) + (atr / max(price, 1.0) * 10.0)
        ))
    ...
    def get_runtime_snapshot(self, price_brl: float) -> dict[str, Any]:
        ...
        dist  = float(self.last_signal_context.get("distancia_percentual") or 0.0)
        atr   = float(self.last_signal_context.get("atr")      or 0.0)
        gate  = float(self.last_signal_context.get("atr_gate") or 0.0)

        conf = {
            "ema_above_sma": ema9 > ema21 if ema21 > 0 else False,
            "sma_slope_up":  slope > 0,
            "low_above_ema": True,
            "distancia_ok":  (dist > 0) and (atr > 0) and (abs(ema9 - ema21) > gate),
        }
        conf["veredito"] = "Confluencia confirmada" if sum(conf.values()) >= 3 else "Aguardando confluencia"

        # Snapshot de debug da estratégia
